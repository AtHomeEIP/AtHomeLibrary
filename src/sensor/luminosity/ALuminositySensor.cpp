...
static const PROGMEM uint16_t
    minimalLuxValues[ALuminositySensor::Room_Type::ARCHIVES + 1] = {
        0,   100, 150, 100, 300, 500, 100, 300,
        300, 500, 500, 500, 750, 500, 200};
}

ALuminositySensor::ALuminositySensor()
    : _value({ISensor::ISensorScale::ZERO,
              {utility::units::UNIT::LUX, utility::units::PREFIX::BASE_UNIT},
 ...
 *
 * Values are from french norm NF EN 12464-1
...
const ISensor::ISensorValue &ALuminositySensor::getSample() {
  ...
  _value.estimate = (_lux < _min || _lux > _max) ? ISensor::ISensorScale::ONE
                                                 : ISensor::ISensorScale::TEN;
  return _value;
}

void ALuminositySensor::setThresholds(
    const ISensor::ISensorThresholds &thresholds) {
