...
void MQ2GasSensor::setPin(int pin) { _pin = pin; }

int MQ2GasSensor::getPin() const { return _pin; }
 ...
 */
int MQ2GasSensor::getLPG() const { return _values.lpg; }
 ...
 */
int MQ2GasSensor::getCO() const { return _values.co; }
 ...
 */
int MQ2GasSensor::getSmoke() const { return _values.smoke; }
 ...
 *
 * Given the voltage across the load resistor and its resistance, the resistance
 * of the sensor could be derived.
 */
 ...
 * It use MQResistanceCalculation to calculates the sensor resistance in clean
 * air and then divides it with RO_CLEAN_AIR_FACTOR. RO_CLEAN_AIR_FACTOR is
 * about 10, which differs slightly between different sensors.
 */
 ...
 * of ppm) of the line could be derived if y(rs_ro_ratio) is provided. As it is
 * a logarithmic coordinate, power of 10 is used to convert the result to
 * non-logarithmic value.
 */
int MQ2GasSensor::MQGetPercentage(float rs_ro_ratio, float *pcurve) {
  return static_cast<int>(utility::math::pow<float>(
      10, ((utility::math::log<float>(rs_ro_ratio) - pcurve[1]) / pcurve[2]) +
              pcurve[0]));
}
