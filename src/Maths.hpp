 ...
 * int main(void) {
 *   constexpr float a =
 * athome::utility::math::static_integrate<float>(square<float>, 0, 5);
 ...
 */
template <typename T>
constexpr T static_floor(T a) {
  return a - (a - static_cast<int32_t>(a));
}
 ...
 */
template <typename T>
constexpr T static_ceil(T a) {
  return (a - static_cast<int32_t>(a)) ? (a + 1) - (a - static_cast<int32_t>(a))
                                       : a;
}

template <typename T>
static constexpr T static_rec_sqrt(T ix, double x0) {
  return (static_abs<T>(2 * x0) < M_E)
             ? (x0 * x0) - ix
             : ((static_abs<T>((x0 - ((x0 * x0 - ix) / (2 * x0))) - x0) <=
                 (0.00000001 *
                  static_abs<T>((x0 - ((x0 * x0 - ix) / (2 * x0))))))
                    ? (x0 - ((x0 * x0 - ix) / (2 * x0)))
...
/**
 * Return the power of the x value (1st parameter) by the y value (2nd
 * parameter)
 */
template <typename T>
constexpr T static_pow(T x, T y) {
  return (y < 1)
             ? NAN
             : ((static_cast<int32_t>(y) == 1) ? x
                                               : x * static_pow<T>(x, y - 1));
}
 ...
 */
template <typename T>
constexpr T static_exp(T x) {
  return static_pow<T>(M_E, x);
}
 ...
 */
template <typename T>
constexpr T static_exp2(T x) {
  return static_pow<T>(2, x);
}

template <typename T>
static constexpr T static_rec_integrate(T (*f)(T x), T a, T b, int n, T step,
                                        T area, size_t i = 0) {
  return (i < n) ? static_rec_integrate(f, a, b, n, step,
 ...
 *
 * The optional 4th parameter allows to set the number of subdivisions used to
 * integrate the function, thus directly affects the precision of the result
 */
template <typename T>
...
constexpr T static_log(T z, size_t iterations = 150, T n = 0) {
  return (z > 0 && n < iterations)
             ? ((n == 0)
                    ? 2 * (((1 / (2 * n + 1)) *
                                static_pow<T>((z - 1) / (z + 1), 2 * n + 1) +
                            static_log(z, iterations, n + 1)))
                    : ((1 / (2 * n + 1)) *
                           static_pow<T>((z - 1) / (z + 1), 2 * n + 1) +
                       static_log(z, iterations, n + 1)))
             : 0;
...
T log(T z) {
  ...
  for (T n = 0; n < 150; n++) {
    res += (1 / (2 * n + 1)) *
           athome::utility::math::pow<T>((z - 1) / (z + 1), 2 * n + 1);
  }
  return res * 2;
}
...
static constexpr T static_m_precision(T x, T p, T m = 1) {
    return (x * static_pow(2, m) > static_pow<T>(2, p / 2)) ? m :
static_m_precision<T>(x, p, m + 1);
}
...
template <typename T>
constexpr T static_agm(T x, T y) { return (M_PI / 4) * ((x + y) / static_k((x -
y) / (x + y))); }

// Was supposed to use arithmetic geometric approximation
template <typename T>
constexpr T static_ln(T x) {
    return M_PI / (2 * static_agm(1, static_pow(2, 2 - static_m_precision(x,
23)) / x)) - static_m_precision(x, 23) * LN2;
} */
}  // namespace math
}  // namespace utility
}  // namespace athome
