...
namespace athome {
    namespace time {
        ...
        void FakeRTC::setCurrentDateTime(const ITime::DateTime &date) {
            _date = date;
        }
        ...
        const ITime::ISO8601DateTime &FakeRTC::getDateTime() {
            ...
            if (!elapsed) {
                return _date; // No seconds elapsed, only milliseconds so return current date
            }
            uint8_t tmp = elapsed % 60; // Get the number of seconds to add
            if (tmp) {
                addSeconds(tmp); // If we have a non-null number of seconds in the current minute
            }
            ...
            if (tmp) {
                addMinutes(tmp); // If we have a non-null number of minutes in the current hour
            }
            ...
            addDays(elapsed); // Add remaining days (can't be years in theory, as milliseconds are coded on 32-bit and will overflow after about 50 days
            return _date;
        }
        ...
        void FakeRTC::addDays(uint8_t days) {
            ...
            else if (_date.month == 1 || _date.month == 3 || _date.month == 5 || _date.month == 7 || _date.month == 8 ||
                    _date.month == 10) {
                if (new_days > 31) {
                    _date.day = new_days - 31;
                    _date.month++;
