...
namespace sensor {
/**
 * Base class derived from athome::sensor::ISensor allowing to use an analog
 * sensor.
 *
 * Warning: This class is only able to sample an analog sensor, not to interpret
 * its value.
 *
 * Thus, `getEstimate` method will always return
 * athome::sensor::ISensor::ISensorScale::ZERO if this method is not overriden
...
class AnalogSensor : public ISensor {
 public:
  ...
  ~AnalogSensor();
  /**
   * Return a pointer on a `uint16_t` variable, so the memory area returned by
   * the pointer is 2 bytes long or equivalent to an array of 2 `uint8_t`.
   *
   * Examples of use:
   *
   * \code{.cpp}
   ...
   *   if (pointer == nullptr) {
   *     // Invalid pointer -> might be caused by a sample error from the
   * sensor.
   *     // Can't happen with base AnalogSensor class but could happen from
   * other athome::sensor::ISensor derived classes, so it's good practice to
   * always check returned pointer. return;
   *   }
   ...
   *
   *   printf("%hu\n", *full_pointer); // Or Serial.println(*full_pointer) for
   * example on Arduino printf("%hu\n", *c_pointer); // Or
   * Serial.println(*c_pointer) for example on Arduino printf("%hu\n", value);
   * // Or Serial.println(value) for example on Arduino
   * }
   * \endcode
   */
