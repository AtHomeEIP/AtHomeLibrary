...
#include "AtHomeConfig.h"
    ...
    !defined(DISABLE_THERMISTOR_SENSOR)
 ...
 *
 * TODO: Need to update it to use the static version of log function once
 * implemented
 */
template <uint32_t R1, uint32_t R2, uint32_t R3, size_t PREC = 1024,
          uint8_t VREF = 5, uint32_t RREF = 10000>
...
#warning Not yet implemented for your platform
    ...
    sample = (1 / (_a + (_b * utility::math::log<float>(resistance)) +
                   (_c * utility::math::pow<float>(
                             utility::math::log<float>(resistance), 3)))) -
             273.15;
 ...
 private:
  ...
  static constexpr float _c =
      ((_lambda3 - _lambda2) / (_l3 - _l2)) * (1 / (_l1 + _l2 + _l3));
  static constexpr float _b =
      _lambda2 - (_c * (utility::math::static_pow<float>(_l1, 2) + (_l1 * _l2) +
                        utility::math::static_pow<float>(_l2, 2)));
  static constexpr float _a =
      _y1 - ((_b + (utility::math::static_pow<float>(_l1, 2) * _c)) * _l1);
};
}  // namespace sensor
}  // namespace athome
