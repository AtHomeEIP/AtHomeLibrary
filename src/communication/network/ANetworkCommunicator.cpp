...
ANetworkCommunicator::ANetworkCommunicator(ip::tcp_host *host)
  ...
  (host == nullptr) ? memset(&_host, 0, sizeof(_host))
                    : memcpy(&_host, host, sizeof(_host));
}

ANetworkCommunicator::~ANetworkCommunicator() {}

const ip::tcp_host &ANetworkCommunicator::getHost() const { return _host; }
...
bool ANetworkCommunicator::isHostConfigured() { return _hostConfigured; }
}  // namespace communication
}  // namespace athome
