...
\textcolor{keywordtype}{void} my\_func\_reading\_my\_sensor(ISensor &my\_sensor) \{
  ...
  \textcolor{keywordflow}{if} (pointer == \textcolor{keyword}{nullptr}) \{
    \textcolor{comment}{// Invalid pointer -> might be caused by a sample error from the}
sensor.
    \textcolor{comment}{// Can't happen with base AnalogSensor class but could happen from}
other \mbox{\hyperlink{classathome_1_1sensor_1_1_i_sensor}{athome::sensor::ISensor}} derived classes, so it\textcolor{stringliteral}{'s good practice to}
\textcolor{stringliteral}{always check returned pointer. return;}
\textcolor{stringliteral}{  \}}
\textcolor{stringliteral}{  // Can access value from casting pointer to its original uint16\_t type:}
\textcolor{stringliteral}{  uint16\_t *full\_pointer = reinterpret\_cast<uint16\_t *>(pointer);}
\textcolor{stringliteral}{  // Or by casting it in C-style:}
\textcolor{stringliteral}{  uint16\_t *c\_pointer = (uint16\_t *)pointer;}
\textcolor{stringliteral}{  // Or by accessing it as an array of 2 bytes:}
\textcolor{stringliteral}{  uint16\_t value = ((pointer[0]) << 8) | pointer[1];}
\textcolor{stringliteral}{}
\textcolor{stringliteral}{  printf("%hu\(\backslash\)n", *full\_pointer); // Or Serial.println(*full\_pointer) for}
\textcolor{stringliteral}{example on Arduino printf("%hu\(\backslash\)n", *c\_pointer); // Or}
\textcolor{stringliteral}{Serial.println(*c\_pointer) for example on Arduino printf("%hu\(\backslash\)n", value);}
\textcolor{stringliteral}{// Or Serial.println(value) for example on Arduino}
\textcolor{stringliteral}{\}}
\end{DoxyCode}
